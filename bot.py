...
async def quatuor(ctx):
	quatuor = ['Antony','Alex','Romain','Maxime']
	await ctx.send(random.choice(quatuor))
...
@client.event
async def on_message(message):
	if 'happy birthday' in message.content.lower() or 'joyeux an' in message.content.lower() or 'bon an' in message.content.lower():
